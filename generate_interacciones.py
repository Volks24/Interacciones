...
def search_hot_points(Ligand_imput):
    ...
    if ligand_plot == 'Yes':
        img_acceptors = Draw.MolToImage(mol, highlightAtoms=acceptor_atoms, size=(400, 400))
        img_acceptors.save(f"CSV/{folder}/{Ligand_imput.split('.')[0]}_acceptors.png")

        # Gráfico para donadores
        img_donors = Draw.MolToImage(mol, highlightAtoms=donor_atoms, size=(400, 400))
        img_donors.save(f"CSV/{folder}/{Ligand_imput.split('.')[0]}_donors.png")
...
def visualize_rings(mol, ring_data, Ligand_imput):
    ...
    img = Draw.MolToImage(mol, highlightAtoms=ring_atoms, size=(400, 400), kekulize=True)
    img.save(f"CSV/{folder}/{Ligand_imput.split('.')[0]}_aromatic.png")
...
def scripting_vmd(DF_Interacciones,receptor_points,aromatic_lig_df,DF_Lig,Prot,chain,Lig):
    ...
    VDM_TCL = open(f'vmd_{receptor_name}_{Lig_name}.tcl' , 'w')
...
if __name__ == '__main__':
    ...
    try:
        ...
        pass
    try:
        os.mkdir('CSV/{}_{}'.format(receptor_pdb.split('.')[0],Ligand_imput.split('.')[0]))
    except FileExistsError:
        pass
    ...
    DF_Interacciones.to_csv(f'CSV/{folder}/Interaction_{receptor}_{ligand}_all.csv')
    dat_list.append(DF_Interacciones.shape[0])
    ## Filtro Distancia##
    DF_Interacciones = DF_Interacciones[DF_Interacciones['Dist'] < threshold_inter_aro]
    DF_Interacciones.to_csv(f'CSV/{folder}/Interaction_{receptor}_{ligand}_threshold.csv')
    ...
    if not DF_Interacciones.empty:
        ...
        DF_Interacciones.to_csv(f'CSV/{folder}/Interaction_{receptor}_{ligand}_true.csv')
        dat_list.append(DF_Interacciones.shape[0])
    else:
        dat_list.append(0)
    Valores = (dict(DF_Interacciones['Type'].value_counts()))
    ...
    ## Resumen data ##
    out_put_file = 'resumen_inter_all.csv'
    ...
    ## Resumen data ##
    out_put_file = 'resumen_inter_true.csv'
